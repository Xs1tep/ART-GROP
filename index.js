...
function fd(d) { if (!d) return '—'; return new Date(d).toLocaleDateString('ru', {day:'2-digit',month:'2-digit',year:'numeric'}); }
function ft(d) { if (!d) return '—'; return new Date(d).toLocaleTimeString('ru', {hour:'2-digit',minute:'2-digit'}); }
...
async function checkInstallReminders() {
  ...
  for (const e of entries) {
    ...
    const info = `👤 ${e.clientName}\n📞 ${e.phone||'—'}\n🔢 №${e.orderNum||'—'}\n🏭 ${e.manufacturer||'—'}\n📅 ${fd(e.installDate)} в ${ft(e.installDate)}\n📍 ${addrStr(e)}\n💰 ${e.installPayAmount ? e.installPayAmount+' MDL' : '—'}\n💳 ${e.payMethod||'—'}`;
    const dateStr = fd(e.installDate);
    if (diffMin >= 179 && diffMin <= 181) {
      const key = `3h_${e.id}_${dateStr}`;
      if (!sent.has(key)) { sent.add(key); await tgSend(`⏰ <b>Монтаж через 3 часа!</b>\n${info}`); console.log(`3h: ${e.clientName}`); }
    }
    if (diffMin >= 59 && diffMin <= 61) {
      const key = `1h_${e.id}_${dateStr}`;
      if (!sent.has(key)) { sent.add(key); await tgSend(`🚨 <b>Монтаж через 1 час!</b>\n${info}`); console.log(`1h: ${e.clientName}`); }
    }
  }
}
...
async function tgDailyDigest() {
  ...
  const isTomorrow = d => { if (!d) return false; const x = new Date(d); x.setHours(0,0,0,0); return x.getTime()===tomorrow.getTime(); };
  const lines = [];
  entries.forEach(e => {
...
function scheduleDaily(hour, minute, fn) {
  ...
  console.log(`Scheduled ${hour}:${String(minute).padStart(2,'0')}, next in ${Math.round(msUntilNext()/60000)} min`);
}
